    ...
    return (
        <Container className="py-4 mt-5">
            <h1 className={`${theme? 'text-light': 'text-light-primary'} my-5 text-center`}>
                {isEmpty? 'Your Cart is Empty' : ' Cart Items'}
            </h1>
                        ...
                        {items.map((item, index)=>{
                            return(
                                <tr key={index}>
                                    ...
                                    <td>
                                        <Button onClick={()=> updateItemQuantity(item.id, item.quantity - 1)} className="ms-3">-</Button>
                                        <Button onClick={()=> updateItemQuantity(item.id, item.quantity + 1)} className="ms-3">+</Button>
                                        <Button variant="danger" onClick={()=> removeItem(item.id)} className="ms-3">Remove Item</Button>
                                    </td>
                                </tr>
                            )
                        })}
